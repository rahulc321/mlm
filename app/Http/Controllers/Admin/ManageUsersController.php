...
class ManageUsersController extends Controller
{
    ...
    public function emiDefaultersUsers()
    {
        $pageTitle = 'EMI Defaulters';
        $users = DB::table('loan_repayments')
            ->select('user_id')
            ->where('is_emi_paid', 2)
            ->where('is_active', 1)
            ->groupBy('user_id')
        ...
        $users->getCollection()->transform(function ($value) {
            ...
            $user_ref=User::where('id', $user_info->ref_by)->first();
            $user_info->mobile=$user_info->mobile." <br/>  <strong>Ref By: </strong>".$user_ref->full_name." ({$user_ref->username}) ";
            return $user_info;
        });
