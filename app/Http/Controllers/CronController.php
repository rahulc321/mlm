...
class CronController extends Controller
{
            ...
            if ($loan_repayments_failed > 0) {
                ...
                if ($user_members_defaulter_check['is_user_has_emi_defaulter_member']) {
                    if ($user_members_defaulter_check['number_of_loan_users_has_helped_in_team'] >= 6) {
                        ...
                        echo '<br/>user has defaulters ';
                        $is_user_has_defaulters = true;
                    }
                }
                if (!$is_user_has_defaulters) {
                    ...
                    if ($user->status == 1 && $user->plan_id > 0) {
                        ...
                        $current_loop_user_left_downline_info = Mlm::getUserDownline($user, 'LEFT', 1, 1, 0, ['defaulters', 'non_defaulters', 'defaulters_total_bv_sum', 'defaulters_total_count', 'non_defaulters_total_count']);
                        ...
                        $current_loop_user_right_downline_info = Mlm::getUserDownline($user, 'RIGHT', 1, 1, 0, ['defaulters', 'non_defaulters', 'defaulters_total_bv_sum', 'defaulters_total_count', 'non_defaulters_total_count']);
                        ...
                        if (
                            ...
                            if (1) {
                                if ($user_plan_sum > 0) {
                                    ...
                                    // echo $uex->bv_left."<br/>";
                                    //recalculate bv if defaulters found
                                    echo '<br/> total bv of user  in left  ' . $uex->bv_left . '';
                                    if (isset($current_loop_user_left_downline_info['defaulters_total_bv_sum']) && $current_loop_user_left_downline_info['defaulters_total_bv_sum'] > 0) {
                                        echo '<br/> total defaulted bv under user in left  ' . $current_loop_user_left_downline_info['defaulters_total_bv_sum'] . '';

                                        $uex->bv_left = $uex->bv_left - $current_loop_user_left_downline_info['defaulters_total_bv_sum'];
                                        echo '<br/> new bv of user after subtraction in left  ' . $uex->bv_left . '';;
                                    }


                                    echo '<br/> total bv of user in right  ' . $uex->bv_right . '';
                                    if (isset($current_loop_user_right_downline_info['defaulters_total_bv_sum']) && $current_loop_user_right_downline_info['defaulters_total_bv_sum'] > 0) {
                                        echo '<br/> total defaulted bv under user in right  ' . $current_loop_user_right_downline_info['defaulters_total_bv_sum'] . '';
                                        $uex->bv_right = $uex->bv_right - $current_loop_user_right_downline_info['defaulters_total_bv_sum'];
                                        echo '<br/> new bv of user after subtraction in right  ' . $uex->bv_right . '';;
                                    }
                                    // die;
                                    // echo $uex->bv_left."<br/>";die;
                                    // dd($current_loop_user_left_downline_info['defaulters_total_bv_sum'],$uex);
                                    if ($uex->bv_left >= 0 && $uex->bv_right >= 0) {
    ...
    public function dailyTradingIncome()
    {
        ...
        $eligibleUsers = User::where('plan_id', '>', 0)
            ->where('status', '=', 1)
              //->where('username', '=', '2RS540773')
                ->where('username', '=', '2RS557721')
            ->get();
        ...
        foreach ($eligibleUsers as $key => $user) {
            ...
            echo '<br/><br/>Trading income for user:' . $user->username . "<br/>";

            $loan_repayments_failed = $user->loanRepaymentsPending;

            if ($loan_repayments_failed > 0) {
                $user_id_emi_defaulters[] = $user->id;
                echo 'user id emi pending <br/>';
            } else {
                $user_members_defaulter_check = checkUserHasEmiDefaulterInHisTeam($user);
                ...
                if (!$is_user_has_defaulters) {
                    ...
                    foreach ($all_slabs as  $slab) {
                        ...
                        $checkTxnCount = TradingIncome::where('user_id',$user->username)->count();
                        //return $daily_trading_package_income;
                         
                        if($checkTxnCount < $slab->days){
                           // dd($user_all_plans_amount_sum);
                            if ($user_all_plans_amount_sum >= $slab->min_range && $user_all_plans_amount_sum <= $slab->max_range) {
                                ...
                                // New code 
                                $newlogic = $user_all_plans_amount_sum*($slab->return_income_percentage/100);
                                $daily_trading_package_income = return2sureRoundAmount($newlogic, 4);
                                ...
                                $transaction->charge = 0;
                                $transaction->trx_type = '+';
                                $transaction->remark = 'paid_daily_trading_income';
        ...
        // $users_emi_defaulters = DB::table('loan_repayments')
        //     ->select('user_id')
        //     ->where('is_emi_paid', 2)
        //     ->where('is_active', 1)
        //     ->groupBy('user_id')
        //     ->get();
        // $user_id_emi_defaulters = [];
        // foreach ($users_emi_defaulters as $emidef) {
        //     $user_id_emi_defaulters[] = $emidef->user_id;
        // }

        // dd($user_id_emi_defaulters);
                ...
                if (!$is_user_has_defaulters) {
                    ...
                    if ($user_total_business_self >= 100) {
                        ...
                        foreach ($all_reffered_users as $all_reffered_users_k => $all_reffered_users_v) {
                            ...
                            if (!in_array($all_reffered_users_v->id, $user_id_emi_defaulters)) {
                                ...
                                echo 'user has emi defaulter: ' . $all_reffered_users_v->username  . "<br/>";
                            }
                        }
                                ...
                                if ($total_direct_paid_members_100_business >= $all_income_level_item->members) {
                                    ...
                                    if ($all_income_level_item->level == 1) {
                                        ...
                                        $all_reffered_users_members_tmp = $all_reffered_users_members_tmp_level_next_tmp = User::where('ref_by', $user->id)
                                            ->where('plan_id', '>', 0)
                                            ->where('status', '=', 1)
                                            ->whereNotIn('id', $user_id_emi_defaulters)
                                            ->get();
                                        ...
                                        foreach ($all_reffered_users_members_tmp as $all_reffered_users_members_tmpk => $all_reffered_users_members_tmpv) {
                                            ...
                                            $exists = DB::table('level_income_users')
                                            ->where('level', 1)
                                            ->where('from_user_id', $all_reffered_users_members_tmpv->username) // Replace `user_id` with the appropriate column
                                            ->exists();
                                            ...
                                            if (!$exists) {
                                                ...
                                                // if (isset($trading_incomes_tmp_saved)) {

                                                    // New code
                                                    $percent = 50*($all_income_level_item->income/100);
                                                ...
                                                // $level_income_to_give_final = return2sureRoundAmount((($trading_incomes_tmp_saved[$all_reffered_users_members_tmpv->id]['income'] *  $all_income_level_item->income) / 100), 4);
                                                    $level_income_to_give_final = return2sureRoundAmount($all_income_level_item->income);
                                                    ...
                                                    $tmp_latest_user_info = User::select('id', 'balance')->where('id', $user->id)
                                                        ->first();

                                                    DB::table('users')
                                                        ->where('id', $user->id)  // find your user by their email
                                                        ->update(array('balance' => return2sureRoundAmount($tmp_latest_user_info->balance + $level_income_to_give_final, 4)));
                                                    // DB::table('users')
                                                    //     ->where('id', $user->id)  // find your user by their email
                                                    ...
                                                    $transaction->post_balance = return2sureRoundAmount($tmp_latest_user_info->balance + $level_income_to_give_final, 4);
                                                    $transaction->charge = 0;
                                                    $transaction->trx_type = '+';
                                                    // $transaction->remark = 'paid_bv';
                                        ...
                                        if (isset($all_users_in_levels[$all_income_level_item->level])) {
                                            ...
                                            foreach ($all_users_in_levels[$all_income_level_item->level] as $all_reffered_users_members_tmpk => $all_reffered_users_members_tmpv) {
                                                ...
                                                dd($all_reffered_users_members_tmp);
                                                foreach ($all_reffered_users_members_tmp as $all_reffered_users_members_tmpk => $all_reffered_users_members_tmpv) {
                                                    $all_users_in_levels[$all_income_level_item->level + 1][] = $all_reffered_users_members_tmpv->id;
                                        ...
                                        if (isset($all_users_in_levels[$all_income_level_item->level])) {
                                            ...
                                            foreach ($all_users_in_levels[$all_income_level_item->level] as $all_reffered_users_members_tmpk => $all_reffered_users_members_tmpv) {
                                                ...
                                                $all_reffered_users_members_tmp  = User::where('ref_by', $s->id)
                                                    ->where('plan_id', '>', 0)
                                                    ->where('status', '=', 1)
                                                    ->whereNotIn('id', $user_id_emi_defaulters)
                                                    ->get();
    ...
    public function givePlanToUser()
    {
        ...
        $user = User::where('id', $user_to_process)->first();
        $data = Deposit::where('id', $deposit_id)->first();
        ...
        $plan->save();
        $plan = Plan::where('status', 1)->findOrFail($plan->id);
        ...
        $mlm = new Mlm($user, $plan, $trx);
        $mlm->purchasePlanForSomeoneAsLoan($user, $plan, $trx, $user);
        $user = User::find($user->id);
    ...
    public function reward_achievers_generator()
    {
        ...
        foreach ($eligibleUsers as $key => $user) {
            ...
            $user_total_business = Plan::where('plan_created_by_user_id', $user->id)->sum('price');

            if ($user_total_business >= 100) {
                echo "<br/> user has 100+ business : {$user->username}";

                // $users_left_members = Mlm::getAllActivePaidUserOfUserDirectIndirectLeft($user);
                $users_left_members = Mlm::getUserDownline($user, 'LEFT', 1, 1, 1);
                // $users_right_members = Mlm::getAllActivePaidUserOfUserDirectIndirectRight($user);
                $users_right_members = Mlm::getUserDownline($user, 'RIGHT', 1, 1, 1);
                echo "<br/> left active paid members non def " . count($users_left_members);
                echo "<br/> right active paid members non def " . count($users_right_members);
                ...
                echo "<br/> more than 100$ users count left side " . ($left_count);
                ...
                echo "<br/>  more than 100$ users count right side " . ($right_count);



                foreach ($all_clubs as  $club) {

                    $reward_income_achievers_status = DB::table('reward_income_achievers')
                        ->where('reward_id', '=', $club->id)
                        ->where('user_id', '=', $user->id)
                        ->where('is_active', '=', 1)
                        ->count();

                    if ($reward_income_achievers_status === 0) {

                        if ($left_count >= $club->pair && $right_count >= $club->pair) {

                            echo "<br/>  USER eligible for designation {$club->designation} income {$club->reward} ";

                            $left_count = $left_count - $club->pair;
                            $right_count = $right_count - $club->pair;
                            ...
                            if ($reward_income_achievers_status === 0) {
                                DB::table('reward_income_achievers')->insertGetId(
                                    [
                                        // 'action' => 'generation',
                                        'user_id' => $user->id,
                                        'reward_id' => $club->id,
                            ...
                            //give income
                            DB::table('reward_income_user_log')->insertGetId(
                                [
                                    'user_id' => $user->id,

                                    'reward_id' => $club->id,
                                    'reward_amount' => $club->reward,
                            ...
                            $user_to_update = User::where('id', $user->id)

                                ->first();


                            $user_to_update->balance +=  $club->reward;
                            $user_to_update->save();

                            //create transaction
                            $transaction = new Transaction();
                            $transaction->user_id = $user_to_update->id;
                            $transaction->amount =  $club->reward;
                            $transaction->post_balance = $user_to_update->balance;
                            $transaction->charge = 0;
                            $transaction->trx_type = '+';
                            $transaction->remark = 'paid_reward_income';
                            $transaction->details = 'Paid ' .  $club->reward . ' as reward income for achieving designation ' . $club->designation;
                            $transaction->trx =  getTrx();
                            $transaction->save();
                            //give income
                        }
        ...
        $output_of_cron = str_ireplace('<br/>', "\n", $output_of_cron);
        $output_of_cron = str_ireplace('<br>', "\n", $output_of_cron);
        Log::channel('cron_reward_income')->info($output_of_cron);
    }

    public function super_income(){
        $datetime = now();

        $eligibleUsers = User::where('plan_id', '>', 0)
            ->where('status', '=', 1)
            ->where('total_invest', '>=', 50)
            ->get();

            foreach ($eligibleUsers as $user) {
                // Get the current date (without time) in 'Y-m-d' format
                $currentDate = date('Y-m-d');
            
                // Check if a record already exists for the same user and today's date
                $exists = DB::table('super_income')
        ...
        $eligibleUsersNew = User::where('plan_id', '>', 0)
            ->where('status', '=', 1)
            ->where('total_invest', '>=', 50)
            ->get();

        foreach ($eligibleUsersNew as $user) {
            $startDate = Carbon::parse($user->created_at);
            $endDate = $startDate->copy()->addMonth(); // Calculate +1 month from created_at

            // Count how many times their referral code was used within their created_at + 1 month
            $refCount = DB::table('users')
                ->where('ref_by', $user->id)
                ->whereBetween('created_at', [$startDate, $endDate])
                ->count();

            if ($refCount > 0) {
                echo "User ID: " . $user->id . " >>> Referrals: " . $refCount . "<br>";
            }
        }
