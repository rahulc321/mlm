        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <span class="text--danger">@lang('When someone buys this plan, He will get ROI as per the Plan
                        package').</span>
                </div>
    ...
    <div class="modal fade" id="treeComInfoModal" role="dialog" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <span class="text--danger">@lang('When someone from your below tree matching Business volume this
                        plan, You will get this amount as Tree Commission. (tree commission )'). </span>
                ...
                <form action="{{ route('user.plan.purchase') }}" method="post">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <img src="{{ asset('images/' . $gs->qr_code) }}" alt="QR Code" class="qrcode" style="height: 400px;">
                        </div>

                        <div class="form-group">
                            <input type="text" name="txn_id" class="form-control" placeholder="Transaction Id" required> 
                        </div>
    ...
    $(document).on('change', '[name=payment_method]', function() {
        ...
        if ($(this).val() != 'balance' && amount) {
            ...
            if (amount < min_amount || amount > max_amount) {
                $('.gateway-limit').text(
                    `${resource.name} cannot process ${amount} {{ $general->cur_text }} due to transaction limit. Gateway limit: ${min_amount} {{ $general->cur_text }} - ${max_amount} {{ $general->cur_text }}`
                );
